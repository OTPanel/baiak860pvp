...
local muted = createConditionObject(CONDITION_MUTED, CONDITIONID_DEFAULT)

function onSay(player, type, message)
	...
	if player:getCondition(CONDITION_MUTED, CONDITIONID_DEFAULT, CHANNEL_HELP) then
	...
	if playerAccountType >= ACCOUNT_TYPE_TUTOR then
		if string.sub(message, 1, 6) == "!mute " then
			...
			if target then
				if playerAccountType > target:getAccountType() then
					if not target:getCondition(CONDITION_MUTED, CONDITIONID_DEFAULT, CHANNEL_HELP) then
		...
		elseif string.sub(message, 1, 8) == "!unmute " then
			...
			if target then
				if playerAccountType > target:getAccountType() then
					if target:getCondition(CONDITION_MUTED, CONDITIONID_DEFAULT, CHANNEL_HELP) then
						target:removeCondition(CONDITION_MUTED, CONDITIONID_DEFAULT, CHANNEL_HELP)
